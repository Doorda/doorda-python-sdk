...
table_list_df = jdbc_conn.option(
    "dbtable",
    f"(SELECT table_name FROM system.jdbc.tables "
    f"WHERE table_cat = '{args['CATALOG']}' AND "
    f"table_schem = '{args['SCHEMA']}') table_list",
).load()
...
for row in table_list_df.rdd.collect():
    ...
    table_schema_df = jdbc_conn.option(
        ...
        f"WHERE table_name = '{row.table_name}' and column_name != 'data' and column_name != 'value') table_schema",
    ).load()
    ...
    for col in columns_w_complex_types:
        spark_dataframe = spark_df.withColumn(
            col,
            from_json(col, complex_value_map[columns_w_complex_types[col]]),
        )
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        connection_type="s3",
        connection_options={
            "path": args["FILE_OUTPUT_DIRECTORY"] + row.table_name
        },
        format="parquet",
        transformation_ctx="",
    )
